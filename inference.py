...
class ActionGuardrails:
    ...
    def validate_and_correct_action(
        ...
        if action_type == "contain" and action_json.get("containment_action") == "block_ip":
            ...
            if blocking_safe:
                print(f"[GUARDRAIL] Preventing block of known-safe entity. Correcting to dismiss.", file=sys.stderr, flush=True)
                return {
                    "action_type": "contain",
        ...
        if action_type == "contain" and action_json.get("containment_action") == "isolate_machine":
            if any(proc in alert_text.lower() for proc in SafeEntityDatabase.SAFE_PROCESSES):
                print(f"[GUARDRAIL] Preventing isolation of machine with safe process. Correcting to dismiss.", file=sys.stderr, flush=True)
                return {
                    "action_type": "contain",
                    "containment_action": "dismiss"
                }
        ...
        if action_type == "report":
            ...
            if "dismiss" in containment_action.lower():
                if action_json.get("mitre_id") != "None":
                    print(f"[GUARDRAIL] Dismissal should have mitre_id='None'. Correcting.", file=sys.stderr, flush=True)
        ...
        if action_type == "investigate":
            ...
            if any(query in h for h in recent_queries):
                print(f"[GUARDRAIL] Repeated investigation query '{query}'. Forcing contain.", file=sys.stderr, flush=True)
                return {
                    "action_type": "contain",
                    "containment_action": "dismiss"
                }
...
def improved_get_model_action(
    ...
    for attempt in range(1, LLM_MAX_RETRIES + 1):
        try:
            ...
            print(f"[DEBUG] Attempt {attempt}/{LLM_MAX_RETRIES}: JSON parse failed: {e}", file=sys.stderr, flush=True)
            if attempt < LLM_MAX_RETRIES:
                time.sleep(delay)
                delay *= 2
        except Exception as e:
            print(f"[DEBUG] Attempt {attempt}/{LLM_MAX_RETRIES}: Error: {e}", file=sys.stderr, flush=True)
            if attempt < LLM_MAX_RETRIES:
                time.sleep(delay)
                delay *= 2
...
def log_start(task: str, env: str, model: str) -> None:
    print("============================================================", flush=True)
    print(f"Running scenario: {task}", flush=True)
    print("============================================================", flush=True)
            ...
            try:
                ...
                for step_idx in range(1, MAX_STEPS + 1):
                    ...
                    try:
                        ...
                        error = str(e)
                        print(f"[ERROR] env.step failed: {e}", file=sys.stderr, flush=True)
                        reward = 0.001
                        done = True
                    ...
                    try:
                        ...
                        print(f"[ERROR] _obs_to_dict failed: {e}", file=sys.stderr, flush=True)
                        done = True
                ...
                print(f"[FATAL ERROR] Episode {episode} failed: {e}", file=sys.stderr, flush=True)
                score = min(max(score, 0.001), 0.999)
            finally:
                log_end(
                    success=success,
                    steps=episode_steps,
                    score=score,
                    rewards=rewards if rewards else [0.001],
                )
...
if __name__ == "__main__":
    try:
        ...
        try:
            print(f"[FATAL UNEXPECTED ERROR] {e}", file=sys.stderr, flush=True)
        except Exception:
            pass
        try:
            sys.stdout.flush()
